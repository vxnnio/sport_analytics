...
def get_food_upload_folder() -> str:
    # 存在 <project>/static/uploads/food
...
def upload_food():
    if request.method == 'POST':
        ...
        athlete_id = request.form.get('athlete_id', 1, type=int)
...
def delete_food(photo_id: int):
    ...
    if os.path.exists(file_path):
        try:
            ...
            current_app.logger.warning(f"刪除檔案失敗（但 DB 已刪）：{file_path}，原因：{e}")
